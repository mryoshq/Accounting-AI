...
interface AddInternalInvoiceProps {
  isOpen: boolean;
  onClose: (id?: number, matchedCustomer?: CustomerPublic) => void;
  onCancel: () => void;
  prefillData?: Partial<InternalInvoiceCreate> & { 
    customer?: string; 
    ice?: string;
    matchedCustomer?: CustomerPublic;
  };
  invoiceProcess?: boolean;
}

const AddInternalInvoice: React.FC<AddInternalInvoiceProps> = ({ isOpen, onClose, onCancel, prefillData, invoiceProcess }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      amount_ht: 0,
      currency_type: "MAD",
      customer_id: customers?.data[0]?.id || 0,
      project_id: 0,
      ...prefillData,
    },
  });

  const findBestMatchingCustomer = useMemo(() => {
    return (customerName: string, ice: string) => {
      if (!customers) return null;
      
      console.log(`Searching for customer: ${customerName}, ICE: ${ice}`);
      
      if (ice === "00000") {
        console.log("ICE is 00000, matching only by name");
        const results = fuzzysort.go(customerName, customers.data, {
          key: 'name',
          threshold: -10000,
        });
        ...
        const exactMatch = customers.data.find(c => c.ice === ice);
        if (exactMatch) {
  ...
  useEffect(() => {
    if (prefillData) {
      Object.entries(prefillData).forEach(([key, value]) => {
        if (value !== undefined && key !== 'matchedCustomer') {
          setValue(key as keyof InternalInvoiceCreate, value as any);
        }
      });
  
      if (prefillData.customer && prefillData.ice && customers) {
        const matchedCustomer = findBestMatchingCustomer(prefillData.customer, prefillData.ice);
        if (matchedCustomer) {
          setValue('customer_id', matchedCustomer.id);
          setIsCustomerPrefilled(true);
          prefillData.matchedCustomer = matchedCustomer;
        } else {
          setIsCustomerPrefilled(false);
        }
      }
    }
  }, [prefillData, setValue, customers, findBestMatchingCustomer]);

  const mutation = useMutation({
    mutationFn: (data: InternalInvoiceCreate) =>
      InternalInvoicesService.createInternalInvoice({ requestBody: data }),
    onSuccess: (data) => {
      const invoiceId = data.id;
      showToast("Success!", "Internal Invoice created successfully.", "success");
      reset();
      onClose(invoiceId, prefillData?.matchedCustomer);
    },
  ...
  const onSubmit: SubmitHandler<InternalInvoiceCreate> = (data) => {
  ...
  const isPrefilled = (fieldName: keyof InternalInvoiceCreate) => {
  ...
  const handleCustomerCreated = (newCustomer: any) => {
    queryClient.setQueryData(["customers"], (old: any) => {
      const updatedData = [...(old?.data || []), newCustomer];
      return { ...old, data: updatedData };
    });
  ...
  const form = (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <FormControl mt={4} isRequired isInvalid={!!errors.customer_id}>
        ...
        <Flex>
          <Select
            ...
            bg={isCustomerPrefilled ? "yellow.500" : "transparent"}
            flex="1"
            mr={2}
          ...
          <Button onClick={() => setIsAddCustomerModalOpen(true)} variant='outline'>
            Add New
          </Button>
        </Flex>
        {errors.customer_id && (
  ...
  return (
    <Modal isOpen={isOpen} onClose={onCancel} size={{ base: "sm", md: "md" }} isCentered >
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Add Internal Invoice</ModalHeader>
        <ModalCloseButton />
        <ModalBody pb={6}>
          {form}
          {isAddProjectModalOpen && (
...
export default AddInternalInvoice;
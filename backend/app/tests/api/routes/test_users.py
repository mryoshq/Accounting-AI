...
def test_create_user_new_email(
    ...
    with (
        ...
        created_user = r.json()
        user = get_user_by_email_db(session=db, email=username)
        assert user
...
def test_get_existing_user(
    ...
    assert 200 <= r.status_code < 300
    api_user = r.json()
    existing_user = get_user_by_email_db(session=db, email=username)
...
def test_delete_user_current_super_user_error(
    ...
    super_user = get_user_by_email_db(session=db, email=settings.FIRST_SUPERUSER)
    assert super_user
